...
export default function Projecttable() {
  ...
  const handleUpload = async (file, fileType) => {
    ...
    const response = await fetch(
      "http://localhost:8080/project/uploadProjectfile",
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const handleConfirmDelete = () => {
    if (postIdToDelete) {
      fetch(`http://localhost:8080/project/delete/${postIdToDelete}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
          "Content-Type": "application/json",
  ...
  const handleEditformSubmit = () => {
    ...
    fetch(`http://localhost:8080/project/update/${projectID}`, {
      method: "PATCH",
      headers: {
        Authorization: `Bearer ${localStorage.getItem("token")}`,
        ...
        <Table sx={{ minWidth: 650 }} aria-label="simple table">
          ...
          >
            <TableRow>
              ...
              {/* ajay commeneted this for remove coloms from small devices */}
            ...
            {projects !== null &&
              ...
              projects.map((row) => (
                ...
                >
                  ...
                  <TableCell component="th" scope="row">
                    <Link to={`/project/${row._id}`}>{row.ProjectTitle}</Link>
                  </TableCell>
                  ...
                  {/* ajay deleted this for remove edit and delete options in small devices */}
                  {/* <TableCell align="left">
                    <Button
                      variant="outlined"
